  ...
  main() {
    ...
    chrome.runtime.onInstalled.addListener(async () => {
        ...
        try {
            ...
            updateContextMenu(CONTEXT_MENU_ID_COPY, `Copy as Markdown (${shortcutCopy})`, ['selection' as chrome.contextMenus.ContextType]);
            updateContextMenu(CONTEXT_MENU_ID_PICK, `Pick Element to Copy as Markdown (${shortcutPick})`, ['page' as chrome.contextMenus.ContextType, 'selection' as chrome.contextMenus.ContextType]);
    ...
    function updateContextMenu(id: string, title: string, contexts: chrome.contextMenus.ContextType[]) {
        chrome.contextMenus.remove(id, () => {
            if (chrome.runtime.lastError) {/* Ignore */}
            chrome.contextMenus.create({
                id: id,
                title: title,
                contexts: contexts as [chrome.contextMenus.ContextType, ...chrome.contextMenus.ContextType[]],
            }, () => {
                 if (chrome.runtime.lastError) {
                    console.error(`Error creating/updating context menu ${id}:`, chrome.runtime.lastError.message);
                 } else {
    ...
    function sendMessageToTab(tabId: number | undefined, action: string, triggerSource: string) {
        if (tabId === undefined) {
            console.error(`Cannot send message for action "${action}" (triggered by ${triggerSource}): Tab ID is undefined.`);
            return;
        }
        ...
        chrome.tabs.sendMessage(tabId, { action: action }, (response: any) => {
            if (chrome.runtime.lastError) {
                console.error(`Error sending message for action "${action}" (triggered by ${triggerSource}):`, chrome.runtime.lastError.message);
            } else {
                console.log(`Message sent for action "${action}" (triggered by ${triggerSource}), response received:`, response);
            }
        });
    }
  },
